...
class Roomfirst extends PureComponent {
  ...
  isFinished = () => {
    ...
    if (isMapActive && isFoodActive && isLetterActive && isDiaryActive) {
      const timer = setTimeout(() => {
        this.props.history.push(`/invite?name=${this.state.name}&time=${this.state.time}`);
        clearTimeout(timer);
      }, 4000);
    }
  };
  ...
  componentDidMount() {
    ...
    let letter1 = localStorage.getItem("letterActive");
    if (letter1 === "true") {
      this.setState({ isLetterActive: true });
      localStorage.setItem("letterActive",false)
    }
    
    let  food1 = localStorage.getItem("foodActive");
    if (food1 === "true") {
      this.setState({ isFoodActive: true });
      localStorage.setItem("foodActive",false)
    }
    let diary1 = localStorage.getItem("diaryActive");
    if (diary1 === "true") {
      this.setState({ isDiaryActive: true });
      localStorage.setItem("diaryActive",false)
    }
    let  map1 = localStorage.getItem("mapActive");
    if (map1 === "true") {
      this.selectAction("map");
